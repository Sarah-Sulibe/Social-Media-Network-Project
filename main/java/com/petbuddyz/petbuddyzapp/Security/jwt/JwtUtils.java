...
public class JwtUtils {
    ...
    public String generateRefreshToken(Authentication authentication) {
        ...
        refreshToken.setUsername(userPrincipal.getUsername());
        refreshToken.setExpiryDate(new Date((new Date()).getTime() + jwtExpirationMs * 2));
    ...
    private Key key() {
        ...
        if (keyBytes.length < 32) {
            throw new IllegalArgumentException(
                    "JWT secret key is too short. Make sure it's at least 256 bits (32 bytes).");
        }
